...
4. **Only scoring chains are modeled** (~65–70% of chains dropped) — profiles are attack-only; defensive structure and turnover pressure are invisible.
...
|---|----------|-------|----------|--------|-----|
| E1 | INFO | Elo "winner" = sign of model `actual_delta`, not the real scoreboard result | `Core/elo_engine.py` | **Kept by design (2026-08-09):** deliberate tactical-Elo semantics. Watch items: (a) tier/ladder labels (`ELITE`…`REBUILDING`) present it as absolute strength while the model is attack-only (see E3) — consider a "TACTICAL" qualifier on ladder/tier graphics; (b) `elo_weight` blending in `generate_round_images.py:123` / `server.py:374` partially double-counts the delta signal (Elo is a smoothed function of historical deltas) — a results-trained Elo would add independent signal if ever wanted; scores remain available in `match_info`. Optional A/B experiment (delta-Elo vs results-Elo walk-forward) discussed, declined. **Update (2026-08-09):** server ladder simulation consolidated onto the single `EloEngine.elo_update()` formula — the second results-based Elo path is gone. | No action — accepted. |
| E2 | ~~MEDIUM-HIGH~~ **FIXED (2026-08-09)** | No normalization of edge weights | `Core/engine_data.py:159-171` (accumulate), `211-219` (average by match count only) | ~~A team with more scoring chains (or longer chains) gets larger absolute matrix values, so `delta` partly measures chain *volume* not *efficiency*. Also chain-length bias~~ — fixed by normalizing each match's matrix by `sum(abs(w))` at storage time. G6 net delta: +15.494 → +0.1238 (same winner). **Follow-on:** the ×12 margin magic (`server.py:481`) and Elo `margin_mult` are now calibrated to the old magnitude scale — re-fit when E4 is done. | Done. |
| E3 | **MEDIUM** | Only SCORE-outcome chains are profiled | `Core/engine_data.py:141-142` (`if not has_score: continue`) | 65–70% of chains (turnovers, stoppages, defensive sequences) never shape a team's profile. Profiles describe attack-only ball movement; defensive weakness and turnover patterns are invisible to the matchup. | Document as an explicit modeling decision, or extend profiling to TURNOVER/STOPPAGE outcomes with appropriate decay. |
| E4 | ~~MEDIUM~~ **FIXED (2026-08-09)** | Margin model is a magic constant | `server.py:481` | ~~Hardcoded ×12 scaling~~ — replaced with regression fitted on the exact features the server uses: `margin = 6.23 + 107.49·net_delta + 3.68·elo_diff` (412 matches, r=0.540; `analyze_margins.py` now emits all three coefficients into config). First attempt was fit on net_delta only but applied to combined_score — that let Elo gaps blow margins to 164+ pts; caught and re-fitted (predicted |margin| now mean 16.7 / max 60.6). **Also fixed in the same family:** probability `net_delta*80` → fitted logistic `P(home)=sigmoid(0+3.787·net+0.0036·elo)` (b0=0 by design — no venue advantage; Brier 0.2023 vs 0.2066) and total-score `shots*4` → league-average total 159.3. Fit tools: `analyze_margins.py`, `fit_calibration.py`. | Done. |
...
| E6 | **LOW-MEDIUM** | Backtest evaluation is weak | `backtest_2025.py`, `backtest_2026.py` | Winner-only accuracy on a tiny sample (2026: rounds 0–4); no calibration (Brier), no margin error, no statistical significance; `net_delta == 0` silently counts as away win. | Walk-forward evaluation with calibration + margin RMSE; report n and confidence intervals. |
| E7 | **LOW** | Pre-season (round 0) matches pollute history | `engine_scraper.py:165` (`range(0,25)`), `engine_data.py:81-82` (only `>24` excluded) | Round-0 (pre-season) games enter team histories and the 25-match window, shaping regular-season profiles. | Exclude round 0 from profiling, or add a config flag. |
| E8 | **LOW** | `norm_x`/`norm_y` columns contain raw coordinates | `engine_scraper.py:139` (writes `sx, sy, sx, sy`) | Columns are mislabeled; any consumer trusting them gets raw meters. Grid math re-normalizes internally so predictions are unaffected. | Write true normalized values or drop the columns. |
| E9 | **LOW** | Draw handling inconsistencies | `Core/models.py:98-101` (`winner` returns home on tie); `backtest_*.py:40` (tie → away) | Inconsistent tie semantics across code paths. | Centralise a `winner()` helper with explicit DRAW handling. |
| E10 | **LOW** | Legacy HUD retains the old (buggy-class) goal handling | `Core/engine_visualizer.py:51` (`if end == 'SCORE' and score < 0: target = 'AWAY_G'`) | `Core/main.py` `predict` command (the CLI default) still draws through `TacticalHUD`, which has the pre-fix goal-only swap. | Route `predict` through `MatchupVisualizer` or retire `TacticalHUD`. |
