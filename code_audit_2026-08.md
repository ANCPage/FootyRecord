...
| E2 | **MEDIUM-HIGH** | No normalization of edge weights | `Core/engine_data.py:159-171` (accumulate), `211-219` (average by match count only) | A team with more scoring chains (or longer chains) gets larger absolute matrix values, so `delta` partly measures chain *volume* not *efficiency*. Also chain-length bias: decay is positional within chain but nothing normalizes per-chain total. | Normalize each match's matrix by total weight (or by possession count) before averaging. |
...
| E4 | **MEDIUM** | Margin model is a magic constant | `server.py:481` (`predicted_margin = round(combined_score * 12.0)`) | The dashboard's margin/scores are a hardcoded ×12 scaling. `analyze_margins.py` already fits a proper linear regression (`Expected Margin = slope·Tactical Score + intercept`) — the result is never wired into the server. | Expose the fitted slope/intercept from `analyze_margins` via config; fall back gracefully. |
