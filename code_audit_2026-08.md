...
| E4 | ~~MEDIUM~~ **FIXED (2026-08-09)** | Margin model is a magic constant | `server.py:481` | ~~Hardcoded ×12 scaling~~ — replaced with regression fit from `analyze_margins.py` (412 matches 2024-25, r=0.538): `margin = 144.14·combined + 6.19`, stored in `config.py` (`margin_slope`/`margin_intercept`) for re-fitting. G6 example: 1 pt → 24 pts. **Follow-on (also fixed):** probability `net_delta*80` → fitted logistic `P(home)=sigmoid(0.304+3.787·net+0.0036·elo)` (Brier 0.2023 vs 0.2066; adds home-ground intercept; fit shows 1 net_delta ≈ 1050 elo pts, not 80) and total-score `shots*4` → league-average total 159.3 (the shots feature was 100% clamp-bound post-E2, r=0.002). Fit tool: `fit_calibration.py`. | Done. |
